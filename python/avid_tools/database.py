...
def update_md5(
    conn: Connection, relative_file_path: Path | str, avid_dir: Path = Path(), raise_unaffected: bool = True
):
    rows_affected = (
        conn.execute(
            "update files set md5 = ?, size = ? where path = ?",
            [
                file_md5(avid_dir / relative_file_path),
                (avid_dir / relative_file_path).stat().st_size,
    ...
    if raise_unaffected:
        assert rows_affected, f"File {relative_file_path} was not affected!"
