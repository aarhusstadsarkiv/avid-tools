...
def _ignore_args(func):
    options = [
        ...
        click.option(
            "--ignore_table_regexes", type=str, multiple=True, default=[], help="Regex(es) of tables to ignore"
        ),
        click.option(
            "--ignore_column_regexes", type=str, multiple=True, default=[], help="Regex(es) of columns to ignore"
        ),
    ]
...
def cmd_encode(
    ...
    ignore_table_regexes: list[str],
    ignore_column_regexes: list[str],
):
    """
...
def cmd_search(
    ...
    """
    feo2xmlprobe.search_database(
        db_path, text, ignore_tables, ignore_columns, ignore_table_regexes, ignore_column_regexes
    )
